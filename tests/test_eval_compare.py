...
def test_metrics_registry_covers_spec():
    ...
    # spec §4.4 report rows (all required keys including medians)
    required = {
        "mean_ttft_ms", "median_ttft_ms", "p90_ttft_ms", "p99_ttft_ms",
        "mean_itl_ms", "median_itl_ms", "mean_tpot_ms",
        "mean_e2el_ms", "p99_e2el_ms",
        "output_throughput", "request_throughput",
    }
    assert required <= keys, f"Missing required metrics: {required - keys}"
